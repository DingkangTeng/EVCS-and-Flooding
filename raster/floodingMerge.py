...
class floodingMerge:
    ...
    def mergeAll(self, savePath: str, mainBand: int):
        ...
        # merge的时候注意，不同国家可能有重复，不能单纯的相加，要去除并集(用max)
        ...
        with rio.open(os.path.join(savePath, "_All_merge.tif"), 'w', **outMeta) as dst:
            dst.write(mosaic)
        for ds in datasets:
            ds.close()
    ...
    def mergeOneCountry(self, country: str, savePath: str, mainBand: int) -> str:
        ...
        for file in files:
            print("Processin {}/{}".format(i, n)) #优化多线程后改成进度条
        ...
        print("Mosaicing all rasters to new raster...")
        if mainBand in [1, 5]:
...
if __name__ == "__main__":
    floodingMerge(r"F:\\Flooding").mergeOneCountry("AFG", "test", 2)