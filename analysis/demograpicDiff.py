...
def calDiff(df: pd.DataFrame, diffType: str) -> tuple[pd.DataFrame, list[str]]:
    ...
    if diffType == "gender":
        subdf["Male - Female"] = subdf["A_Male_diff"] - subdf["A_Female_diff"]
        ...
        for col in ["children", "young", "elderly"]: # "A_children"
            ...
            subdf[result] = subdf["A_middle_diff"] - subdf["A_{}_diff".format(col)]
            x.append(result)
    elif diffType == "evcs":
...
if __name__ == "__main__":
    demograpicDiff(r"C:\\0_PolyU\\test\\iso3.csv", "age")